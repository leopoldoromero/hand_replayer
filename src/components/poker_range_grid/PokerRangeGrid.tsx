...
const allHandsSortedByStrength = [
  // Pocket Pairs (13)
  'AA','KK','QQ','JJ','TT','99','AKs','AKo','AQs','88','AJs','KQs','ATs','AQo','77',
  'KJs','KTs', 'A9s','AJo','KQo','QJs','QTs','66', 'JTs','A8s','55','ATo','KJo','KTo','A5s',
  'A4s','QJo','QTo','JTo','A7s','A6s','A3s','A2s','K9s','Q9s','A9o','K8s','K7s','J9s','A8o','K9o', 
  '44','K6s','A7o','33','Q8s','Q7s','K5s','A5o', 'Q9o','T9s','K4s','T8s','98s','A6o','K3s','K8o',
  'J8s','J7s','22','A4o','K2s','Q6s','Q5s','K7o','A3o','J9o','T7s','Q8o','T9o','J6s','A2o','Q4s',
  'Q3s','J5s','87s','K6o','K5o','J8o','T6s','Q2s','Q7o','J4s','97s', '96s','K4o','76s','T8o','J3s',
  '98o','Q6o','J7o','K3o','K2o','T5s', 'T4s','J2s','95s','Q5o','65s','T3s','T7o','86s', '85s','75s',
  'Q4o','J6o','97o','54s','87o','T2s','94s','Q3o','84s','93s','J5o','64s','74s','Q2o','92s','J4o','53s',
  '43s','32s','96o', '86o','76o','73s','83s','63s','T6o','J3o','T5o','82s','J2o','T4o','95o','52s','62s',
  '72s','42s','85o','65o','75o','T3o','T2o','94o','54o','84o','64o','93o','74o','92o','53o','83o','63o',    
  '73o','43o','82o','52o','62o','42o','32o','72o'  
];
...
const PokerRangeGrid = () => {
  ...
  const combosCalculator = (hand: string) => {
    ...
    if (hand?.includes(POCKET_PAIRS_ABREVIATE_sIMBOL)) {
      const allPocketPairs = pokerHands.map((row, i) => pokerHands[i][i]).reverse();
      return TOTAL_POCKETS - allPocketPairs.indexOf(hand.slice(0, 2)) * POCKET_PAIR_COMBOS;
    }
  ...
  const getRangeString = (selected: Array<string>) => {
    ...
    const sortedPocketPairs = pocketPairs.map((el) => allPocketPairs.indexOf(el)).sort((a,b) => a-b).map((index) => allPocketPairs[index])
    const lowestPP = sortedPocketPairs?.length ? sortedPocketPairs[0] : '';
    const allHighestPp = lowestPP ? allPocketPairs.slice(allPocketPairs.indexOf(lowestPP)) : []
    const pocketPairsRange = (pocketPairs?.length && allHighestPp?.length ===  pocketPairs?.length) ? `${lowestPP}+` : pocketPairs;
    const nonPocketHands = selected.filter((h) => !(h.length === 2 && h[0] === h[1]));
    return pocketPairsRange ? [...[pocketPairsRange], ...nonPocketHands] : nonPocketHands
  };
  ...
  const handleSliderChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let i = 0;
    const handsToSelect = [];
    while(calculatedPercentage < newPercentage && i < allHandsSortedByStrength.length) {
      handsToSelect.push(allHandsSortedByStrength[i])
      calculatedPercentage += (combosCalculator(allHandsSortedByStrength[i]) * 100) / calculateCombos();
      i++
    }
  ...
  return (
    <Container>
      ...
      <Block display="flex" align="center" mt="s">
        <Text as="strong" fontSize="base" weight="semiBold" mr="xs">Selected Range:</Text> 
        <Text 
        whiteSpace="nowrap" 
        overFlow="hidden" 
        textOverFlow="ellipsis" 
        fontSize="medium"
        customStyles={{maxWidth: '250px'}}>{getRangeString(selectedHands).join(", ")}</Text>
      </Block>
    </Container>
  );
};
