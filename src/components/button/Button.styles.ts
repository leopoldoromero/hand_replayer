  ...
  switch (variant) {
    case 'text':
      return css`
        ...
        border: 0;
        color: ${disabled
          ? (p) => p.theme.color.mediumSilver
          : (p) => p.theme.color[color ?? 'black']};
        &:hover {
...
>`
  ...
  align-items: center;
  ${(p) =>
    p.$size && p.$variant !== 'icon'
      ? setSize(p.$size, p.$variant)
      : `padding: 0`};
